...
-- Insert some default tags for suppliers (they can customize/delete these)
-- Note: These will be added per-supplier basis through the UI
...
CREATE POLICY "Suppliers can view notes for their orders"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM orders
      WHERE orders.order_id = order_notes.order_id
      AND orders.supplier_id IN (
        SELECT id FROM suppliers WHERE owner_id = auth.uid()
      )
    )
...
CREATE POLICY "Suppliers can view tag assignments for their orders"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM orders
      WHERE orders.order_id = order_tag_assignments.order_id
      AND orders.supplier_id IN (
        SELECT id FROM suppliers WHERE owner_id = auth.uid()
      )
    )
